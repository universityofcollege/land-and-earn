...
function PacketsView({ data, onPacket, operate }: { data: DashboardData; onPacket: (packet: PacketSummary) => void; operate: Operate }) {
  ...
  const filtered = data.packets.filter((packet) => {
    const matchesFilter = filter === "all" || (filter === "open" ? !["paid", "approved"].includes(packet.status) : filter === "deadline_risk" ? !["paid"].includes(packet.status) && packet.dueDate <= data.settings.invoiceDeadline : packet.status === filter);
